...
class S3Container extends Container {
     ...
     * @param  {Object} domains Domains.
     */
    constructor(bucket, accessKeyId, secretAccessKey, region, domains = {}) {
        super();
        this.bucket = bucket;
    ...
    /**
     * @return {Promise<FileInfo[]>} List of files in the container.
     */
    listFiles() {
        var self = this;
        ...
        this._s3.listObjects({
            ...
            if (result.IsTruncated) {
                ...
                var keys = objects.map((f) => {
                    return new FileInfo(f.Key, f.Size, new Date(f.LastModified), new Date(f.LastModified));
                });
                return done(keys);
            }
        });
